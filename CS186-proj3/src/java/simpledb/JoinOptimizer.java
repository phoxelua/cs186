...
public class JoinOptimizer {
    ...
    public JoinOptimizer(LogicalPlan p, Vector<LogicalJoinNode> joins) {
        this.p = p;
        this.joins = joins;
        this.subsets = new HashMap<Integer, Set<Set<LogicalJoinNode>>>();
        this.currSize = 0;
    }
    ...
    public Set<Set<LogicalJoinNode>> enumerateSubsets(Vector<LogicalJoinNode> v, int size) {
        Set<Set<LogicalJoinNode>>els = new HashSet<Set<LogicalJoinNode>>();
        if (this.subsets.containsKey(size)){
            return this.subsets.get(size);
        }
        else if (this.subsets.containsKey(size-1)){
            els = this.subsets.get(size-1);
            for (int i = size-1; i < size; i++) {
                Set<Set<LogicalJoinNode>>newels = new HashSet<Set<LogicalJoinNode>>();
                for (Set<LogicalJoinNode> s : els) {
                    for (LogicalJoinNode t : v) {
            ...
            this.subsets.put(size, els);
            this.currSize = size;
            return els;
        }
        els.add(new HashSet<LogicalJoinNode>());
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        // 8.            if (cost(plan) < cofilterSelectivitiesst(bestPlan))
        ...
        for (int i = 1; i < joins.size() + 1; i++) {  
            for (Set<LogicalJoinNode> s : enumerateSubsets(joins, i)) {
                ...
                for (LogicalJoinNode sPrime : s) { 
                    subPlan = computeCostAndCardOfSubplan(stats, filterSelectivities, sPrime, s, bestPlan.cost, optjoin);
        ...
        // Vector<LogicalJoinNode> best = optjoin.getOrder(new HashSet<LogicalJoinNode>(joins));
        // Enumeration vEnum = best.elements();
        // while(vEnum.hasMoreElements()){
        //     System.out.println(vEnum.nextElement().toString() + " ");
        // }
        // System.out.println("PRRRRRRRRRRRRINNNT");
